...
compare_richness <- function() {
    ...
    for (group in metadata$taxa_name) {
        match_taxa <- db_call_function("match_taxa", schema="api", taxa_name=group)
        n_taxa <- nrow(match_taxa)
        if (n_taxa <= 1) {
            taxa_counts <- c(taxa_counts, n_taxa)
        } else {
            obs_summary <- db_call_function("obs_summary", schema="atlas_api", min_year = 1950, max_year = 2020, region_type = "hex", taxa_keys = match_taxa$id_taxa_obs)
